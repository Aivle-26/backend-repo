...
@Slf4j
@Component
public class HttpReassignmentAgentClient implements ReassignmentAgentClient {

    private static final String RECOMMEND_PATH = "/api/v1/risk/assignee-reassignment";

    private final RestClient restClient;

    public HttpReassignmentAgentClient(AiServerProperties properties) {
    ...
    @Override
    public AiReassignmentResponse recommend(AiReassignmentRequest request) {
        try {
            AiReassignmentResponse response = restClient.post()
                    .uri(RECOMMEND_PATH)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(request)
                    .retrieve()
                    .body(AiReassignmentResponse.class);
